...
| D16 | A slashed bond must not be slashed again | `challenge()` rejects when `bond_intact` is `False`; `resolve()` skips the bond leg when the bond is already gone, still returns the challenger's deposit in full, and records the skipped leg in the payouts | Found in PM review of CP3c. Actions recorded before a slash stay OPEN and challengeable, so a second out-of-mandate verdict paid the full bond a second time. All mandates' bonds share one contract balance, so the second payout came out of another principal's money. Reachable in the demo scenario |
